...
export const calculateHaversineKm = (
  ...
  if (
    !isValidCoord(lat1, lon1) ||
    !isValidCoord(lat2, lon2)
  ) {
    return 0;
  }
...
const getPartnerPinColor = (
  partner: NearestPartnerResponse['partner']
) => {
...
const PartnerCard = ({
  ...
  : "—"}
      ...
      <View style={styles.partnerContent}>
        ...
        {category ===
          'IMPLEMENTING_ASSISTANCE_CENTRE' && (
          <CategoryBadge
            label={t(
        ...
        {category ===
          'NEARBY_FINANCIAL_SERVICE_POINT' && (
          <CategoryBadge
            label={t(
        ...
        {partner.institution_type && (
          <View style={styles.typeBadge}>
            <Text style={styles.typeBadgeText}>
              {partner.institution_type.replace(
                /_/g,
                ' '
              )}
            </Text>
          ...
          partner.supported_schemes.length > 0 && (
            <View style={styles.supportedSchemes}>
              ...
              <Text style={styles.supportedText}>
                ...
                {partner.supported_schemes.length >
                  3 && (
          ...
          {isValidCoord(
            ...
            <Pressable
              ...
              style={({ pressed }) => [
                ...
                pressed &&
                  styles.directionsButtonPressed,
              ]}
              accessibilityRole="button"
...
> = ({
  ...
  const [routeInfo, setRouteInfo] =
    useState<OSRMRouteInfo | null>(null);

  const [routeStatus, setRouteStatus] =
    useState<string | null>(null);

  const [isMapLocked, setIsMapLocked] =
    useState(false);
  ...
  const filteredPartners = useMemo(() => {
    ...
    if (
      ...
      return partners.filter((item) => {
        ...
        return (
          partner?.institution_type ===
            'RSETI_TRAINING_INSTITUTE' ||
          partner?.partner_type ===
            'RSETI_TRAINING_INSTITUTE' ||
          item.service_type?.includes(
            'TRAINING'
          ) ||
          partner?.service_type?.includes(
            'TRAINING'
          )
        );
      });
    ...
    if (
      selectedCategoryFilter === 'VERIFIED'
    ) {
      return partners.filter(
        (item) =>
          item.partner?.verification_status ===
          'VERIFIED_OFFICIAL'
      );
    }
    ...
    (
      ...
      const validPartners = (
        ...
        .filter(
          (item) =>
            item?.partner &&
            typeof item.partner.latitude ===
              'number' &&
            typeof item.partner.longitude ===
              'number' &&
            isValidCoord(
        ...
        .map((item) => {
          const latitude =
            item.partner!.latitude as number;

          const longitude =
            item.partner!.longitude as number;
      ...
      if (validPartners.length > 0) {
        const firstId =
          validPartners[0].partner.partner_id;
  ...
  const fetchPartners = useCallback(
    async (
      ...
      try {
        const data =
          await partnerApi.getNearestPartners(
            queryLat,
            queryLng,
            radiusKm,
            schemeId,
        ...
        console.error(
          'Failed to load nearest partners:',
          error
        );
    ...
    useCallback(async () => {
      ...
      try {
        ...
        if (!isValidCoord(lat, lng)) {
          setErrorMsg(
            'Received invalid GPS coordinates.'
          );
          return;
        }
  ...
  const handleSearch = useCallback(
    async () => {
      ...
      if (!cleanInput) {
        setErrorMsg(
          'Please enter a valid PIN code, district, or state name.'
        );
        return;
      }
      ...
      let searchLat: number | null =
        null;

      let searchLng: number | null =
        null;
      ...
      try {
        ...
        setErrorMsg(
          error?.response?.data?.detail ||
            'Failed to fetch nearby partners.'
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const recenterMap = useCallback(() => {
    ...
    if (
      ...
      if (valid.length > 0) {
        const lats = valid.map(
          (item) =>
            item.partner.latitude!
        );

        const lngs = valid.map(
          (item) =>
            item.partner.longitude!
        );
      ...
      >
        {[
          ...
          },
          {
            ...
            label: t(
              'partnerLocator.categoryAssistance',
              'Assistance Centres'
            ),
          },
          {
            ...
            label: t(
              'partnerLocator.categoryTraining',
              'Training / EDP'
            ),
          },
          {
      ...
      {schemeId && (
        <View style={styles.schemeBanner}>
          <View style={styles.schemeBannerTop}>
            ...
            <View
              style={
                styles.authorizedCount
              }
            >
              <Text
                style={
                  styles.authorizedCountText
                }
              >
                {filteredPartners.length}{' '}
                {t(
                  'partnerLocator.authorized',
                  'Authorized'
                )}
              </Text>
            </View>
        ...
        selectedPartnerObj && (
          ...
          >
            <View>
              ...
              >
                {routeInfo.distanceKm.toLocaleString()}{' '}
                km • ~
                {formatDuration(
            ...
            >
              ...
              <Text
                style={
                  styles.drivingBadgeText
                }
              >
                {t(
                  'partnerLocator.driving',
                  'Driving'
                )}
              </Text>
            </View>
